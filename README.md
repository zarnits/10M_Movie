...
#### 개별 영화 별 일별 관객수 추이
- 누적 관객수가 높을 수록 유사한 패턴(주말에 높고 주중에 낮았다가 다시 주말에 높아지고, 첫 주가 가장 높고 점차 낮아지는)을 보였다. 
...
초반 흥행을 다른 영화가 가져가버렸다. 


### 장르

#### 장르별 영화 분포
- 천만 영화 기준 장르 자체는 다양했다. 다만, 드라마 액션 어드밴쳐 sf가 집단(천만, 비천만, 200위까지 상위권)을 가리지않고 높은 점유율을 보였다. 

#### 장르별 누적 관객수
- 장르별 차이를 그래프에서 보이는 것 같았지만, 검정결과 유의미한 차이를 발견하지 못했다. 

#### 장르+국적
- 특정 장르의 국가적 편중을 볼 수 있었다. 
+ EX) sf-미국, 드라마-한국
...
이러한 모습은 MCU의 스파이더맨 시리즈에서도 찾아볼 수 있었다.
...
특히 어밴져스는 2편 이후 계속 천만 돌파에 성공했다.
윈터솔져는 단일 주인공체제지만, 시빌 워는 캡틴 아메리카와 아이언 맨의 복수 주인공 체제라서 그런지 둘 사이 큰 관객 수 차이를 보인다. 


#### 특별관
- 일반(필름,디지털,2D), 특별관(3D,4D,IMAX 등) 
- 아바타1, 2와 트랜스포머3만 특별관 관람 비중이 50%가 넘지만, 천만에 성공한 것은 아바타가 유이하다. 트랜스포머3는 770만을 달성했다. 그 외에는 20%p 가까이 차이나는 모습을 보인다. 


#### 상영등급
- 상영등급에 따른 영향
- 청불의 최고 등수는 내부자로 62위 700만에 그쳤다. 그리고 전체 관람가도 겨울왕국1, 2와 알라딘 이후 109위 미녀와 야수에서 다시 등장한다. 그 외의 15세와 12세 관람가는 비슷한 분포를 보인다.
다만, 각 등급별과 청불 비청불 간 두 번의 차이 검정에서 표본수 부족으로 추측되는 문제로 유의미한 결과를 얻지 못했다.     


#### 감독
천만 영화의 감독
천만 영화 전 후로 유의미한 차이가 있으나, 천만을 쉽게 달성한다거나 흥행을 보장하지는 않는 모습을 보인다. 

### 그 외의 요소

#### 날씨
강수량과 양의 상관관계가 있으나 유의미한 관계는 없다. 
기온과는 꽤 밀접한 관련이 있었다. 평균기온이 높아질 수록 타일 수와 더불어 색이 짙어졌다. 이와 반대로 평균 기온 10도 내외에서는 다른 평균 기온대에 비해서 상단 타일 수 자차에서 큰 차이를 보였다. 영하권을 살펴보면, 영하권인 날 자체가 다른 기온 대에 비해 확연히 줄지만, 따스한 날씨에 비하면 높은 관객 동원력을 확인해볼 수 있었다. 

#### 티켓 가격
추세 변환, 가격 변경일 전 후 2주간 총 한 달간의 평균 등을 비교해본 결과, 9000원 이전까지는 티켓 가격이 상승하더라도 관람객 수가 느는 모습을 볼 수 있었지만, 그 이후로는 급락 이후 회복하는 패턴을 보이다, 코로나 이후 쉽사리 회복하지 못하는 모습을 보인다. 특히 9000원 시기 천만 영화가 가장 많았으나, 만원으로 인상한 이후, 천만 영화의 등장 간격이 늘어난 것을 볼 수 있었다. 
유의미한 지점은 8000원 인상 이후 해운대, 아바타가 관객 회복을 도왔다면, 13000원 시기에 접어들어서는 모두 차기작인 작품만이 천 만을 달성할 수 있었다.  
만원을 넘어서 13000원 시대에 접어들면서 지불 의향 가격과 실제 표 가격 차이가 급격히 줄어들면서 입소문을 타거나 대박난 전작이 있는 등 소비자 후생이 보장된 영화가 아니라면 천만 영화는 고사하고 흥행 자체가 힘든 모습을 보인다. 

#### 국민 총 소득
문화여가지출과 오락 문화비에서 유의미한 관계 및 인과를 확인하였다. 다만, 같이 비교한 거시 경제 지표(국내총생산, 가계총처분가능소득, 국민처분가능소득 등)이 상호 연관성이 강하기 때문에 분산팽창계수에서 6000을 넘어 변수간 높은 상관관계를 확인할 수 있었다. 
특이점으로, 연간 총 관객수와 문화여가지출률 둘과 문화여가지출률과 거시 경제 지표들 사이에서 높은 상관계수를 보이지만, 정작 연간 총 관객수와 거시 경제 지표들 사이에서는 관계성이 없는 모습을 보였다. 

#### 요일
요일별로 확연한 차이를 보였다. 크게 토일, 수목금, 월화 로 나눌 수 있는데, 토요일이 가장 높고 그 뒤를 일요일이 바짝 추격하는 모습이라면, 수목금은 금수목 순으로 나열되지만 그 사이 차이가 주말과의 차이만큼 크지 않다. 그리고 월화는 가장 낮은 관객 동원력을 보인다. 수목금에서 금요일은 다음날이 휴일이라서 퇴근 후 영화를 감상하는 경우가 많은 것으로 보이고, 금요일이 주된 개봉일도 아니고 주말과 같은 가격을 받으면서도 높은 관객수를 보이는 것이 인상적이다. 수요일과 목요일이 유사한 것은 개봉일 효과로 보인다. 
요일별 최고치에서 큰 차이가 보이지 않는 것은 공휴일이 골고루 포함된 결과로 보인다.
