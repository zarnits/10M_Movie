...
#### 요일
요일별로 확연한 차이를 보였다. 크게 토일, 수목금, 월화 로 나눌 수 있는데, <br/>
토요일이 가장 높고 그 뒤를 일요일이 바짝 추격하는 모습이라면, 수목금은 금수목 순으로 나열되지만 <br/>
그 사이 차이가 주말과의 차이만큼 크지 않다. 그리고 월화는 가장 낮은 관객 동원력을 보인다. <br/>
수목금에서 금요일은 다음날이 휴일이라서 퇴근 후 영화를 감상하는 경우가 많은 것으로 보이고, <br/>
금요일은 주된 개봉일도 아니고 주말과 같은 가격을 받으면서도 높은 관객수를 보이는 것이 인상적이다. <br/>
수요일과 목요일이 유사한 것은 개봉일 효과로 보인다. <br/>
